...
void main() {
  test(
    'AppDatabase self-heals on batchUpsertDeckAndCardsTransaction if stability is missing',
    () async {
      ...
      final card = await (appDb.select(
        appDb.flashcards,
      )..where((c) => c.id.equals('card_1'))).getSingle();
  ...
  test(
    'AppDatabase self-heals flashcards table if already at version 5 but missing columns',
    () async {
