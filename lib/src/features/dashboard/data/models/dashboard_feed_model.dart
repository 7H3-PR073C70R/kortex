...
abstract class CuratedCourseModel with _$CuratedCourseModel {
  ...
  factory CuratedCourseModel.fromJson(
    Map<String, dynamic> json,
  ) => CuratedCourseModel(
    id: (json['id'] as String?) ?? '',
    courseCode: (json['courseCode'] ?? json['course_code'] ?? '') as String,
    title: (json['title'] ?? '') as String,
    department: (json['department'] ?? 'General Studies') as String,
    totalMaterials:
        (json['totalMaterials'] ?? json['total_materials'] as num?)?.toInt() ??
        15,
    hasActivePastPapers:
        (json['hasActivePastPapers'] ??
            json['has_active_past_papers'] as bool?) ??
        true,
    iconName: (json['iconName'] ?? json['icon_name'] ?? 'school') as String,
    colorHex: (json['colorHex'] ?? json['color_hex'] ?? '#6366F1') as String,
    pdfDownloadUrl:
